...
from ui.components.tooltips import add_tooltip_to_widget, CloudTooltip
from ui.components.storage_tooltips import get_storage_tooltip
from ui.components.bucket_detail_view import BucketDetailView
...
class ModernStorageView(QWidget):
    ...
    def _init_list_view(self):
        """Initialize list view with clear section hierarchy"""
    ...
    def _create_header(self) -> QWidget:
        ...
        tooltip_btn.setStyleSheet(f"""
            QPushButton {{
                background: {Colors.STORAGE_BG};
                border: 1px solid {Colors.STORAGE};
                border-radius: 6px;
                padding: 6px 12px;
                color: {Colors.ACCENT};
                font-size: {Fonts.SMALL};
    ...
    def _create_stat_card(self, value: str, label: str, color: str) -> QFrame:
        """Create stat card with dominant metrics"""
        card = QFrame()
    ...
    def _render_buckets(self):
        ...
        for bucket in buckets:
            # Calculate total size from bucket's total_size attribute
            ...
            card_data = {
                'icon': '📦',
                'name': bucket.name,
                'id': bucket.name,  # Use bucket name as ID
                'status': 'available',
                'details': {
                    'Objects': f"{bucket.object_count} objects",
                    'Total Size': size_str,
                    'Created': bucket.created_at if isinstance(bucket.created_at, str) else 'N/A',
                    'Region': 'us-east-1',
                },
                'actions': ['delete']
            }
    ...
    def show_bucket_details(self, bucket_name: str):
        """Show enhanced bucket details with object management"""
        ...
        # Create enhanced detail view with drag-and-drop and object preview
        detail_page = BucketDetailView(
